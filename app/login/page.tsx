...
function errorMessage(code?: string, sec?: string): string | null {
  if (!code) return null;
  if (code === "empty") return "아이디를 입력해주세요.";
  if (code === "locked") return `시도 너무 많음. ${sec ?? "60"}초 후 다시 시도하세요.`;
  if (code === "invalid") return "존재하지 않는 아이디입니다.";
  if (code === "no_email") return "이메일이 등록되어 있지 않습니다. 학교 관리자에게 문의해주세요.";
  if (code === "use_initial") return "첫 로그인 단계로 돌아가주세요.";
  return "로그인에 실패했습니다.";
}
